...
\subsection{Alcance}
   De manera superficial el alcance es la porción del programa donde una variable toma efecto y altera el valor que esta posee. La implementación del alcance difiere para la máquina $\Hs$ y la máquina $\Js$. Tomemos como ejemplo la siguiente expresión de \textsf{MinHS}
    ...
    \begin{definition}[Alcance estático] En los lenguajes que implementan el alcance estático, la porción del programa donde una variable está enmarcada es la región en la cual se encuentra definida. \\\\ Los marcadores pueden ser bloques de indentación, marcadores de lugar como el operador \textsf{let, in, end}, cuerpo de una función etc\footnote{Definición extraída de \hyperlink{2}{[2]} y \hyperlink{107}{[107]}}.
    \end{definition}
    
    \begin{definition}[Alcance dinámico] En un lenguaje con alcance dinámico, el alcance de una variable es todo el programa y se toma la última asignación hecha a la misma cuando se requiera evaluar\footnote{Definición extraída de  \hyperlink{2}{[2]} y \hyperlink{107}{[107]}}.
    \end{definition}

\subsection{Cerraduras}
    Los alcances de las máquinas $\Hs$ y $\Js$ suponen un problema dado que los resultados obtenidos al evaluar la misma expresión son distintos.\\\\ Esto se debe de corregir para la definición de la máquina $\Js$ mediante la introducción de cerraduras, que encapsulan el contexto de evaluación y la expresión para la que dicho contexto será aplicado cuando se evalúe.\\\\
    ...
    \scalemath{0.8}{
            \begin{array}{c}
                ...
                \\
                \inference{}{app(fix(f.\ll \E_f,x.e_1\gg),\square); P\ |\ \E \prec v \rightarrow_{j} \E; P\ |\ x\leftarrow v;f\leftarrow fix(f.\ll \E_f,x.e_1 \gg );ec_f\succ e_1}
            \end{array}
}
        \]
        \bigskip
