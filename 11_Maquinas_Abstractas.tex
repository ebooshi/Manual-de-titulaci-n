...
\[
 \scalemath{0.85}{
	...
	%  let(\square,x.let(4,y.if(x,sum(y,1),res(y,1)))):\diamond & \succ & \textsf{false} & \rightarrow_{\Hs} \\
	 % let(\square,x.let(4,y.if(x,sum(y,1),res(y,1)))):\diamond & \prec & \textsf{false} & \rightarrow_{\Hs} \\
	 ...
	 % let(\square,y.if(\textsf{false},sum(y,1),res(y,1))):\diamond & \succ & 4  & \rightarrow_{\Hs} \\
	  %let(\square,y.if(\textsf{false},sum(y,1),res(y,1))):\diamond & \prec & 4  & \rightarrow_{\Hs} \\
...
La introducción del concepto de ambientes, nos permitirá almacenar los pares $variable$ / $valor$ en un contexto para optimizar la resolución de una asignación en las expresiones evaluadas por $\Js$.
     ...
     %       \diamond\ |\ \bullet & \succ & let(fun(x ... )) & \rightarrow_{\Js}\\
        %    let(\square, f.let( ...)) : \diamond\ |\ \bullet & \succ & fun(x.sum(x,sum(y,z))) & \rightarrow_{\Js} \\
           % let(\square, f.let( ...)) : \diamond\ |\ \bullet & \prec & fun(x.sum(x,sum(y,z))) & \rightarrow_{\Js} \\
            ...
            %E_3 : E_2 : E_1 :  \bullet : \diamond\ |\ x \leftarrow 1 : z \leftarrow 10 :  y \leftarrow 4 : f \leftarrow fun(x.sum(x,sum(y,z))) : \bullet & \succ & sum(x,sum(y,z)) & \rightarrow_{\Js} \\
