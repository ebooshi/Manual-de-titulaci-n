...
\begin{exercise}[Ejecución de la máquina C]
    ...
    \end{lstlisting}
    Para facilitar la representación y la evaluación de la máquina $\Cs$ vamos a seccionar el programa para tener fragmentos de este, definimos entonces:
\begin{multline*}
A =_{def}\; secu(vardec(Int, i, 1), \\
      secu(vardec(Int, pre, 1), \\
      secu(vardec(Int, post, 1), vardec(Int, fib, 0))))
\end{multline*}
\begin{multline*}    
        B =_{def} secu(assig(fib, sum(pre,post)), \\
		 secu(asig(pre,post), \\
		 secu(asig(post,fib), asig(i,sum(i,1)))
\end{multline*}
	\\*[3pt]
        $ W =_{def} while(lt(i,n),B)) $\\
	\\*[4pt]
        $ F =_{def} fundec(Int,\ fibonacci,\ n:Int.secu(A,secu(W, return(fib)) $\\
	\\*[1pt]
       $ P =_{def} secu(F,vardec(Int, result, call(fibonacci, 3))) $
...
En este caso, el interés no se centra en el resultado producido en el último paso de la evaluación ($\bot$), sino en el estado final de la memoria. El cálculo no devuelve un valor explícitamente; en su lugar, el estado del programa y por consiguiente el resultado quedan impresos en una de las variables que forman parte del contexto final. \\
