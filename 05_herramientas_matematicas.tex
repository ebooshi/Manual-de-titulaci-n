...
\section{Estructuras recursivas e inducción estructural}
    ...
    \begin{itemize}
        ...
        \item La estructura está compuesta por alguno de sus elementos primitivos. Muchas veces estos elementos constituyen la clausula de escape para la recursión inducida por la definición y darán lugar a uno o más axiomas en su representación como regla de inferencia.
        \item La estructura está compuesta por una llamada a alguna de sus funciones constructoras con un elemento nuevo y la estructura misma que se desea definir como argumentos\footnote{Los elementos primitivos y las funciones constructoras de las estructuras recursivas pueden ser diversas, en el caso particular de las listas el único elemento primitivo de la estructura es la lista vacía y la única función constructora es la función \textit{append} representada por la regla \textit{tup}.}. 
    \end{itemize}
    ...
    % Ejercicio 2.1
    \begin{exercise}
        Utiliza la inducción estructural para demostrar que si $t \in \text{A}^*$ entonces:
        \[ t \bullet \varepsilon = t\] 
		 ...
		 $t \bullet \varepsilon = \varepsilon \bullet \varepsilon$\qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad  \qquad \qquad \qquad  \qquad \qquad  (Por hipótesis)\\
            ...
            $\varepsilon$ = $t$. \\\\
            \textbf{Hipótesis Inductiva} $t \bullet \varepsilon = t$ \\\\
            \textbf{Paso Inductivo} Por demostrar para $\<a,t\> \ \forall \ a \in \text{A}$\\ %donde $a \in \text{A}$, $t \in \text{A}^*$ \\
    ...
    \begin{definition}
        Definimos la función para contar ocurrencias de los caracteres 1 ó 0 en una cadena denotada por $numChar$ :: (A $\rightarrow$ A*) $\rightarrow$ Int como sigue\footnote{Definición formulada a partir de \hyperlink{67}{[67]}, \hyperlink{68}{[68]} y \hyperlink{69}{[69]}} :
        \[   
        ...
        \]
    \end{definition}
Cuando se quiera aplicar el principio de inducción a una función o propiedad que se enuncia para mas de un argumento, la inducción se tiene que fijar sobre alguno de sus parámetros.\\\\
En los siguientes ejercicios, cuando interactúen dos elementos $s$, $t$ $\in$ A$^*$ como en ($s \bullet t$), la convención será escoger siempre el parámetro izquierdo (en este caso $s$) para aplicar el principio de inducción, pues la función de concatenación ($\bullet$), está definida para operar sobre el parámetro izquierdo. Ésto simplifica el razonamiento al desarrollar el paso inductivo. \\\\
Ejemplifiquemos esto con el siguiente ejercicio.


    % Ejercicio 2.3
    \begin{exercise}
        Utiliza la inducción estructural para demostrar que sí $c \in \text{A}$ y $s, t \in  \text{A}^*$ entonces:
           \[ numChar(c, s \bullet t) = numChar(c, s) + numChar(c, t)  \]
	 \textbf{Demostración por inducción sobre }$s$.\\
	 %Escogemos el parámetro $s$ para aplicar el principio de inducción pues la función de concatenación ($\bullet$), está definida para el parámetro izquierdo. Esto simplifica el razonamiento al desarrollar el paso inductivo. \\
        ...
        \textbf{Hipótesis Inductiva}  $numChar(c, s \bullet t) = numChar(c, s) + numChar(c, t)$\\

        \textbf{Paso Inductivo} Por demostrar para $\<a,s\> \bullet t \ \ \forall \ t  \in\text{A}^* \text{ y } \forall \ a \in \text{A}$ \\ %\text{ donde } a \in $ A, $s \in $A*\\
    ...
    % Ejercicio 2.4
    \begin{exercise}
        Utiliza inducción estructural para demostrar que si $s,t \in \text{A}^*$, entonces:
            \[ len(s \bullet t) = len(s) + len(t) \]
    	\textbf{Demostración por inducción sobre } $s$.\\
    		 %Escogemos el parámetro $s$ para aplicar el principio de inducción pues la función de concatenación ($\bullet$) y la función \textit{len}, están definidas para operar sobre el parámetro izquierdo. Esto simplifica el razonamiento al desarrollar el paso inductivo. \\
	   ...
	   $len(s \bullet t) = len(\varepsilon \bullet t)$ \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad  (Por la hipótesis)
        ...
        \textbf{Paso Inductivo } Por demostrar para $\<a,s\> \bullet t \ \ \forall \ t  \in\text{A}^* \text{ y } \forall \ a \in \text{A}$\\ %\text{ donde } \textit{a} $\in$ A y \textit{s} $\in$ A*\\
    ...
    \begin{exercise}
        ...
        \textbf{Caso Base} $s = \varepsilon$ \\
            ...
            \textbf{Paso Inductivo} Por demostrar para $\<a, s\> \ \forall \ a \in \text{A}$\\% \textit{a} $\in$ A y \textit{s} $\in$ A* \\
    ...
    \begin{definition}
        Definimos la estructura de datos árboles binarios de tipo A denotados como $Tree_{A}$ con las siguientes reglas\footnote{Definición formulada a partir de \hyperlink{67}{[67]}, \hyperlink{68}{[68]} y \hyperlink{69}{[69]}}:

        \begin{enumerate}
            \item $\emptyset$ $Tree_{A}$.
            \item Si  $t_1$ $Tree_{A}$ y $t_2$ $Tree_{A}$ y $a \in $ A  entonces  $node(a,t_1,t_2)$ $Tree_{A}$.
            \item Son todos.
        \end{enumerate} 
  ...
  \begin{definition}
        Definimos la función para obtener la altura de los elementos de $Tree_{A}$ denotada $height(t)$ :: $Tree_{A}$ $\rightarrow$ Int,  como sigue:\footnote{Definición formulada a partir de \hyperlink{67}{[67]}, \hyperlink{68}{[68]} y \hyperlink{69}{[69]}}:
       \[
            \begin{array}{rcl}
            	height(\emptyset) & = & 0 \\
    ...
    \begin{definition}
        Definimos la función que cuenta el número de nodos de los elementos de $Tree_{A}$ denotada $n_n(t)$ :: $Tree_{A}$ $\rightarrow$ Int,  como\footnote{Definición formulada a partir de \hyperlink{67}{[67]}, \hyperlink{68}{[68]} y \hyperlink{69}{[69]}}:
        \[
            \begin{array}{rcl}
                 n_n(\emptyset)     &  =  & 0  \\
    ...
    % Ejercicio 2.6
    \begin{exercise}
        Demuestra utilizando inducción estructural que si \textit{t} $Tree_{A}$ con altura \textit{k} entonces $n_n(t) \leq 2^k - 1$ \\\\
        \textbf{Caso Base} $t = \emptyset$, $k=0$\\\\
            $n_n(\emptyset) = 0  \leq 2^0 -1 $ = $1 -1$ = 0.\\\\
        \textbf{Hipótesis Inductiva} $n_n(t) \leq 2^k - 1$,  para todo $t$ $Tree_{A}$, donde $height(t)=k$. \\\\
...
\section{Sistemas de transición}
    ...
    \begin{exercise}
        ...
        Para indicar la transición entre estados utilizamos la notación "$\rightarrow$". Por ejemplo si el jugador B anota, la transición se modela como: \[ (\text{B,X,Y}) \rightarrow (\text{B,X,Y+1}) \]  ya que el jugador que anota un punto vuelve a tirar la pelota en el siguiente turno. La partida acaba cuando alguno de los dos logra anotar 10 puntos. \\\\
        Es importante notar que el estado de un juego puede evolucionar hacia distintos estados dependiendo del desarrollo de la partida. Por ejemplo, el estado (B,X,Y) puede transitar al estado (B,X,Y+1) si el jugador B anota el punto, o al estado (A,X,Y) si no anota y el saque pasa al jugador A. Esto implica que la transición $\rightarrow$ no puede modelarse como una función (ya que un mismo estado puede tener más de una imagen), sino como una relación sobre el conjunto de estados, permitiendo que un estado tenga múltiples estados sucesores.\\ 
        ...
        \begin{enumerate}
            ...
            \item  Define la relación de transición (enlista cuales son los casos y como se comporta la relación en cada uno de estos).\\\\
		Definimos la relación de transición $f(s) :: \text{I} \cup \Gamma  \rightarrow \Gamma \cup \text{F}$ a partir de los siguientes casos:\\\\
	...
	\item Define mediante reglas de inferencia la implementación para la relación de transición para cada uno de los casos descritos en el ejercicio anterior.
                \[
                \scalemath{0.81}{
                    \begin{array}{lcl}    
...
Un árbol binario completo es un tipo de árbol binario en el que todos los niveles, excepto posiblemente el último, están completamente llenos, y todos los nodos están lo más a la izquierda posible\footnote{Definición extraída de: \hyperlink{68}{[68]}}.
    ...
    %    Ejercicio 4.6
    \begin{exercise}
        Demuestra utilizando la inducción estructural sobre el tipo de dato $Tree_{A}$ que se cumple la siguiente afirmación:\\\\
        Sea \textit{t} $Tree_{A}$, tal que $t$ es completo, entonces:
...
Demuestra utilizando la inducción estructural sobre el tipo de dato $Tree_{A}$ que se cumple la siguiente afirmación:\\\\
    ...
    \begin{exercise}
        ...
        \begin{enumerate}
            ...
            \item Define la relación de transición $\rightarrow_R$ que indique como se debe transitar entre los estados del sistema.
    ...
    \begin{exercise}
    \label{sec:512}
        Las torres de Hanoi son un rompecabezas clásico cuya solución puede ser escrita de forma recursiva. Los discos de diferentes tamaños se apilan del más grande al más pequeño y el objetivo es moverlos del pivote inicial al pivote final, dónde pivotes = discos = n.\\\\
 Las reglas son  las siguientes: 
        ...
        \begin{center}
            ...
            \end{verbatim}
        \end{center}
	\texttt{Peg} designa el nombre del pivote y \texttt{move} es una tupla ($origen$, $destino$) que modela el movimiento del disco que está en el tope de la pila del pivote origen al pivote destino. \\
