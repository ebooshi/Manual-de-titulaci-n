...
\bigskip
    ...
    \begin{exercise}
        ...
        \]
        \[
            \inference{\inference{\inference{\inference{\texttt{Error}}{\{ x \} \sim y} & \inference{x \in \{x\}}{\{ x \} \sim x}[\textit{(fvn)}]}{\{ x \} \sim (y * x)}[\textit{(fvo)}] & \inference{Error}{\{ x \} \sim z} }{\{ x \} \sim \text{(y * x) + z}}[\textit{(fvo)}]}{\emptyset \sim \texttt{let } x = 1 \texttt{ in } \text{($y$ * $x$) + $z$} \texttt{ end}}[\textit{(fva)}]
        \]
    \end{exercise}
