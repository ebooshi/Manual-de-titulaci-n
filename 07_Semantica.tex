...
\scalemath{0.8}{
   ...
   % Como ambos enfoques solo difieren en el sistema de transición que emplean definiremos los estados de la máquina de transición para ambos a continuación.
    ...
    \begin{definition}Semántica de paso grande para \textsf{EAB}: Se define la relación de transición  $\Downarrow$ mediante las siguientes reglas de inferencia\footnote{Definición formulada a partir de \hyperlink{2}{[2]}, \hyperlink{5}{[5]} y  \hyperlink{12}{[12]} }:
        ...
        \end{description}
         Es importante destacar que los valores de tipo \textit{num[n] y b } Bool si tienen reglas en esta semántica de paso grande.
    \end{definition}
    ...
    \begin{exercise}
        ...
        Síntaxis Concreta:
        \[
            \texttt{let } x \text{ = } \texttt{let } y \text{ =  False} \texttt{ in} \texttt{ if (} y \texttt{ then } 0 \texttt{ else } 1 \texttt{)}  \texttt{ end} \texttt{ in } x + 1 \texttt{ end}
        \]
        Síntaxis Abstracta:
        \[
        ...
        \[
            \scalemath{.73}{
                \inference{ \inference{ \inference{}{False \Downarrow False}[bsbool]  & \inference{}{\textit{if}(y,0, 1\text{)}\textbf{)}[y:=False] \Downarrow 1}[bsift] }{\textit{let}( False,\ y.\textit{if}(y, 0, 1\text{)}\text{)} \Downarrow 1}[bslet] & \inference{}{ \textit{sum}(x,1\textbf{)} [x:=1] \Downarrow 2}[bssum] }{ \textit{let}( \textit{let}( False, y.\textit{if}(y,0,1\text{)}\text{)} ,x.\textit{sum}(x,1\text{))} \Downarrow 2}[bslet]
            }
        \]
    \end{exercise}
    ...
    \begin{exercise}
        ...
        Sintaxis Abstracta:
        \[
            \textit{sum}( \textit{prod}( \textit{sum}( 7, 4 \text{)},4 \textit{)}, \textit{prod}( \textit{sum}( 8, 3\text{)},2 \text{)} \text{)}
        \]
        Evaluación Paso Grande:
        \[
            \scalemath{0.6}{
                \inference{\inference{\inference{\inference{}{7 \Downarrow 7}[bsnum] & \inference{}{4 \Downarrow 4}[bsnum]}{\textit{sum(}7,4\textit{)} \Downarrow 11}[bsum] & \inference{}{4 \Downarrow 4}[bsnum]}{\textit{prod(} \textit{sum(} 7,4 \textit{)}, 4 \textit{)} \Downarrow 44}[bsprod] & \inference{\inference{\inference{}{8 \Downarrow 8}[bsnum] & \inference{}{3 \Downarrow 3}[bsnum]}{\textit{sum(}8,3\textit{)} \Downarrow 11}[bsum] & \inference{}{2 \Downarrow 2}[bsnum]}{\textit{prod(} \textit{sum(} 8,3\textit{)}, 2 \textit{)}  \textit{)} \Downarrow 22}[bsprod] }{\textit{sum(} \textit{prod(} \textit{sum(} 7,4 \textit{)}, 4 \textit{)} , \textit{prod(} \textit{sum(} 8,3\textit{)}, 2 \textit{)}  \textit{)} \Downarrow 66}[bssum]
            }
        \]
    \end{exercise}
