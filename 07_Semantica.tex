...
Adicionalmente estudiaremos los estados y transiciones de la máquina abstracta para definir la semántica operacional del mismo. Este mecanismo se conoce como semántica dinámica y se estudiará en dos paradigmas: de paso grande y de paso pequeño.\\\\
 Por último definiremos la función \texttt{eval} para \textsf{EAB}.
  ...
  \begin{definition}[Semántica estática para capturar expresiones con variables libres en \textsf{EAB}]
    ...
    \[
        \begin{array}{ccc}
            \inference{}{\Delta\sim \textit{bool[b]}}[\textit{(fvb)}]&
			%\inference{}{\Delta\sim \textit{bool[false]}}[\textit{(fvb)}]&
                    ...
                    \inference
                         {
                             \inference
                                 {}
                                 {\emptyset \sim \textit{bool[true]}}[\textit{(fvb)}] 
                             \inference
                                 {
                                     \inference
                                         {
                             ...
                             {\emptyset \sim \textit{if(bool[true], prod(sum(1, 1), 2), 7) }}[\textit{(fvo)}]
	            }           
            \]
        ...
        \[
            	...
            	\inference
			{
				...
				\inference
						...
						\inference
							{
								\inference
									{}
									{\emptyset \sim bool[false]}[\textit{(fvb)}]
								\inference
									{
										\inference
											{}
							...
							{\emptyset \sim \textit{let(} \textit{bool[false],}x.x \textit{) }    }[\textit{(fvo)}] 
						\inference
							{}
							{\emptyset \sim 2}[\textit{(fvn)}] 
						\inference
					...
					{\emptyset \sim \textit{if(} \textit{let(} \textit{bool[false],}x.x \textit{),} 2, 1 \textit{)}}[\textit{(fvo)}]
			}
			{\emptyset \sim \textit{sum(1,} \textit{if(} \textit{let(} \textit{bool[false],}x.x \textit{),} 2, 1 \textit{))}}[\textit{(fvo)}]
            }
        \]
    \end{exercise}
...
\subsection{Semántica operacional}
    ...
    \vspace{1em}
	 ...
	 Los estados iniciales son todas las expresiones cerradas del lenguaje y corresponde a la regla:
            ...
            \item[Estados Finales]  Este conjunto de estados finales está compuesto únicamente por los valores de \textsf{EAB} que formalizamos a continuación\footnote{Los estados que no se pueden reducir más pero que no corresponden a un valor (es decir expresiones mal formadas) se les conoce como estados bloqueados.}. \\\\
	 Definimos una categoría de valores los cuales son un subconjunto de expresiones de \textsf{EAB}. Esta nueva categoría se representa con el juicio $v\ valor$. \\\\
		Para el caso de \textsf{EAB}  se denota con las reglas:
            $$\inference{}{num[n]\ valor}[\textit{(vnum)}] \ \ \ \ \ \inference{}{\textit{bool[b]}\ \ valor}[\textit{(vbool)}]$$ %  \ \ \ \ \ %\inference{}{\textit{bool[false]}\ \ valor}[\textit{(vfalse)}] $$
	...
	\item[Resta]
        ...
        \[
            \scalemath{0.85}{
                \begin{array}{c}
                    \inference{}{if(bool[true],e_1,e_2)  \rightarrow e_1}[$(if_T)$]
                    \quad
                    \inference{}{if(bool[false],e_1,e_2)  \rightarrow e_2}[$(if_F)$]
                    \quad
                \end{array}
            }
        \]
    ...
    \begin{exercise}
        ...
        \[
            \begin{array}{cl}
                &if(bool[false],\ let(prod(99,99),\ x.sum(x,x)),\ 0)\\
                \rightarrow &0
    ...
    \begin{exercise}
        ...
        \[
            \begin{array}{cl}
                &\textit{let}(\textit{if(bool[true]}, 42,0), \textit{x}.\textit{if(F}, \text{41}, x))\\
                \rightarrow &\textit{let}(\text{42}, \textit{x}. \textit{if(F}, \text{41}, x))\\
                \rightarrow &\textit{if(bool[false]}, 41, x)[x:=42]\\
                \rightarrow &\textit{if(bool[false]}[x:=42], 41[x:=42], x[x:=42])\\
                \rightarrow &\textit{if(bool[false]}, 41, 42)\\
                \rightarrow &42\\
            \end{array}
        \]
    \end{exercise}
    ...
    \begin{definition}[Definición de semántica de paso grande para \textsf{EAB}] Se define la relación de transición  $\Downarrow$ mediante las siguientes reglas de inferencia\footnote{Definición formulada a partir de \hyperlink{2}{[2]}, \hyperlink{5}{[5]} y  \hyperlink{12}{[12]} }:
        \begin{description}
            \item[Valores]
            $$\inference{}{num[n] \Downarrow num[n]}[\textit{(bsnum)}] \ \ \ \ \ \inference{}{bool[b] \Downarrow bool[b]}[\textit{(bsbool)}] $$ %\ \ \ \ \ \inference{}{F \Downarrow F}[\textit{(bsfalse)}]$$
            \item[Suma] 
...
Es importante notar que en estos ejercicios estamos haciendo abuso de la notación dado que las reglas para la función de transición de paso grande $\Downarrow$ están definidas para la sintaxis abstracta pero permitimos el uso de la sintaxis concreta para simplificar la notación.
	...
	\bigskip
        Sintaxis abstracta:
        \[
            \textit{if(bool[false]},\textit{sum}(\textit{prod}(3,7\text{)},1\text{)}, \textit{sum}(\textit{prod}(2,7\text{)},1\text{)} \text{)} 
        \]
	\bigskip
                ...
                \inference
		{
			\inference
				{}
				{bool[false] \Downarrow bool[false]}[\textit{(bsbool)}] & 
			\inference
				{
					\inference
						{
		...
		{\textit{if(bool[false]},\textit{sum}(\textit{prod}(3,7\text{)},1\text{)}, \textit{sum}(\textit{prod}(2,7\text{)},1\text{)} \text{ )} \Downarrow 15 }[\textit{(bsiff)}]
            }
        \]
    \end{exercise}
    ...
    \begin{exercise}
        ...
        Sintaxis abstracta:
        \[
            \textit{let}( \textit{let}(bool[false], y.\textit{if}(y, 0, 1\text{)}\text{)} , x.\textit{sum}(x, 1\text{))}
        \]
        Evaluación paso grande:
	\[
	\rotatebox{90}{
		...
		{ 
			\inference
				...
				\inference
					{}
					{bool[false] \Downarrow bool[false]}[\textit{(bsbool)}]  \quad 
					\inference
						{
							\inference
							{
							...
							{
								\textit{if}(bool[false],0, 1\text{)}\text{)} \Downarrow 1
							}[\textit{(bsift)}]
					}
					{
						\textit{if}(y,0, 1\text{)}\text{)}[y:=bool[false]] \Downarrow 1
					}[]
				 }
				 { 
					\textit{let}(bool[false],\ y.\textit{if}(y, 0, 1\text{)}\text{)} \Downarrow 1
				}[\textit{(bslet)}] \quad
				\inference
					{
						\inference
				...
				{
					 \textit{let}( \textit{let}(bool[false], y.\textit{if}(y,0,1\text{)}\text{)} ,x.\textit{sum}(x,1\text{))} \Downarrow 2
				}[\textit{(bslet)}]
            		}
		}
        \]
...
\section{La función eval}
    ...
    La función de evaluación aplica de forma exhaustiva la semántica dinámica hasta llegar a un estado final (en el caso de \textsf{EAB}, un valor).
    \end{definition}
    
    \begin{exercise}
    	Dada la siguiente expresión de nuestro lenguaje \textsf{EAB} utiliza las reglas de transición de la semántica de paso pequeño para evaluarla.
    	\[
    	\texttt{let } y = (5 - 2) \texttt{ in } \texttt{if true then } (y * 3) \texttt{ else } 0 \texttt{ end}
    	\]
    	\[
    	\begin{array}{cl}
    		&\textit{let}(\textit{res}(5,2),\ y.\textit{if}(bool[true],\ \textit{prod}(y,3),\ 0))\\
    		\rightarrow &\textit{let}(3,\ y.\textit{if}(bool[true],\ \textit{prod}(y,3),\ 0))\\
    		\rightarrow &\textit{if}(bool[true],\ \textit{prod}(y,3),\ 0)[y:=3]\\
    		\rightarrow &\textit{if}(bool[true][y:=3],\ \textit{prod}(y,3)[y:=3],\ 0[y:=3])\\
    		\rightarrow &\textit{if}(bool[true],\ \textit{prod}(3,3),\ 0)\\
    		\rightarrow &\textit{prod}(3,3)\\
    		\rightarrow &9
    	\end{array}
    	\]
    	...
    	Por lo tanto,
    	\[
    	\texttt{eval}(\texttt{let } y = (5 - 2) \texttt{ in if true then } (y * 3) \texttt{ else } 0) = 9
    	\]
    \end{exercise}
    ...
    \begin{exercise}
    	Dada la siguiente expresión de nuestro lenguaje \textsf{EAB} utiliza las reglas de transición de la semántica de paso pequeño para evaluarla.
    	\[
    	\texttt{if } ((2 + 3) - 5) \texttt{ then } 1 \texttt{ else } \texttt{let } z = 4 + 1 \texttt{ in } z * z \texttt{ end}
    	\]
    	\[
    	\begin{array}{cl}
    		&\textit{if}(\textit{res}(\textit{sum}(2,3),5),\ 1,\ \textit{let}(\textit{sum}(4,1),\ z.\textit{prod}(z,z)))\\
    		\rightarrow &\textit{if}(\textit{res}(5,5),\ 1,\ \textit{let}(\textit{sum}(4,1),\ z.\textit{prod}(z,z)))\\
    		\rightarrow &\textit{if}(0,\ 1,\ \textit{let}(\textit{sum}(4,1),\ z.\textit{prod}(z,z)))
    	\end{array}
    	\]
    	
    	En este punto la evaluación se bloquea, ya que $0$ no es un valor booleano ($T$ o $F$), por lo que no es posible aplicar ni la regla $(if_T)$ ni $(if_F)$.
    	
    	Por lo tanto,
    	\[
    	\texttt{eval}(e) \text{ no está definida}
    	\]
    \end{exercise}
	...
	\begin{exercise}
		...
		\begin{itemize}
			\item Proporciona la representación en sintaxis abstracta.
