...
\section{Gramáticas y semántica de expresiones}
    ...
    \begin{definition}[Sintaxis de {\sf Expresiones aritmetico-lógicas: EAL}]
        \[
            \begin{array}{rll}
    ...
    % Ejercicio 2.4
    \begin{exercise}
        Evalúa la expresión: \texttt{not} 3 $\textless$ 5 + 7  haciendo uso de la implementación del intérprete anteriormente definida para \textsf{EAL}.
        \vskip 1em
        \begin{lstlisting}
                eval(not 3 < 5 + 7)  =   	                                                                
                not(eval(3 < 5 + 7))  =          	                                                               
    ...
    % Ejercicio 2.5
    \begin{exercise}
        Evalúa la expresión: $3$ * $\texttt{true}$ $ \textgreater$  $8$  haciendo uso de la implementación del intérprete anteriormente definida para \textsf{EAL}.
        \vskip 1em
        \begin{lstlisting}
            eval(3 * true > 8)        =                              
            eval(3 * true) > eval(8)  =            
