...
\begin{definition}[Definición sintaxis concreta para expresiones aritméticas de EAB]]\footnote{Comenzaremos definiendo las operaciones aritméticas e iremos extendiendo las operaciones booleanas con las constantes \texttt{true}, \texttt{false} y los operadores \texttt{if}, \texttt{else} mas adelante en este capítulo.}  En su representación como una gramática libre de contexto\footnote{Definición de \hyperlink{12}{[12]}.}.
    \[
    \begin{array}{rll}
        e & ::= & t \quad |\quad e + t \\
        t & ::= & f \quad |\quad t * f \\
...
\end{definition}
    ...
    %De forma equivalente, la gramática previamente mostrada tiene una representación en un conjunto de juicios lógicos.
     ...
     \begin{definition}[Definición de la sintaxis concreta para expresiones ariméticas de EAB] En su representación como juicios lógicos\footnote{ Definición de \hyperlink{12}{[12]}.}: 
        \[
            \begin{array}{cccc}
            \inference{}{0\;{\text{D}}}&
            \inference{}{1\;{\text{D}}}&
...
%        \]
    ...
    \begin{exercise}
        ...
        \]
        \[
            \inference{\text{$e_1$ B} \longleftrightarrow \textit{$a_1$} \textit{ ASA} \quad \text{ $e_2$ E} \longleftrightarrow \textit{$a_2$ ASA } }
            { \texttt{if $e_1$ then ($e_2$) E} \longleftrightarrow  \textit{if($a_1, a_2$)} \textit{ ASA} }[$(if_s)$]
        \]
        \[
            \inference{\text{$e_1$ B} \longleftrightarrow \textit{$a_1$}  \textit{ ASA } \quad \text{$e_2$ E} \longleftrightarrow \text{$a_2$ ASA } \quad \text{$e_3$ E } \longleftrightarrow \text{$a_3$} \textit{ ASA }}{\texttt{if $e_1$ then ($e_2$) else $e_3$ E } \longleftrightarrow \textit{if($a_1,a_2,a_3$)} \textit{ ASA} }[$(if_e)$]
        \]
    \end{exercise}
...
La sintaxis abstracta de orden superior, abstrae el ligado como parte del metalenguaje con el que se define el \textit{ASA}, es decir el árbol mismo delimita el alcance de la variable ligada a este.\\\\
    ...
    \begin{exercise}
        ...
        \[
            %\inference{\text{$x$ identificador}}{\text{$x$ \textit{ASA}}}[] 
...
\begin{definition}[Alcande de variable] El alcance de una variable es la región de un programa en la que esta obtiene su valor\footnote{Definición formulada de \hyperlink{5}{[5]} y  \hyperlink{12}{[12]}}. \\\\
...
\flushleft{
	\rotatebox{90}{
		\parbox{19cm}{
  			$\textit{let(prod(sum(w, num}[1]), \textit{num}[2]\textit{)}, x.\textit{if(} \text{ T, } w, \textit{ prod(2, x))} \; [ w := \textit{num}[2]]$
		}
	}
}
\end{minipage}
...
\flushleft{
	\rotatebox{90}{
		\parbox{19cm}{
  			$\textit{let(prod(sum(w, num}[1] ),  \textit{num}[2]) \; [w := \textit{num}[2]], x.\textit{if(} \text{ T, } w, \textit{ prod(num}[2], x)) \; [w := \textit{num}[2]] )$
		}
	}
}
\end{minipage}
...
\flushleft{
	\rotatebox{90}{
		\parbox{19cm}{
  			$\textit{let(prod(sum(w, num[1])} [w := \textit{num}[2]], 2 [\textit{var}[w] := 2]), x.\textit{if(} \text{ T} [w := num[2]], w [w := \textit{num}[2]], \textit{ prod(num}[2], x\textit{)}[w := \textit{num}[2]] ))$
		}
	}
}
\end{minipage}
...
\flushleft{
	\rotatebox{90}{
		\parbox{19cm}{
			  $\textit{let(prod(sum(} w [w := \textit{num}[2]] , \textit{num}[1] [w := \textit{num}[2]] ), \textit{num}[2]), x.\textit{if(} \text{ T, } \textit{num}[2], \textit{ prod(num}[2] [w := \textit{num}[2]], x [w := \textit{num}[2]])))$
		}
	}
}
\end{minipage}
...
\flushleft{
	\rotatebox{90}{
		\parbox{19cm}{
 			 $\textit{let(prod(sum(num}[2], \textit{num}[1]), \textit{num}[2]), x.\textit{if( } \text{ T, }  \textit{num}[2], \textit{ prod(num}[2], x)))$
		}
	}
}
\end{minipage}
...
\bigskip
    \begin{exercise}
        Explica por que es necesaria la sintaxis abstracta de orden superior que se introduce con el árbol de sintáxis abstracta \textit{x.t}
    \end{exercise}
