...
\begin{definition}[Definición de sintaxis concreta para expresiones aritméticas de \textsf{EAB}]\footnote{Comenzaremos definiendo las operaciones aritméticas e iremos extendiendo las operaciones booleanas con las constantes \texttt{true}, \texttt{false} y los operadores \texttt{if}, \texttt{else} mas adelante en este capítulo.}  Representación de \textsf{EAB} como una gramática de libre de contexto\footnote{Definición de \hyperlink{12}{[12]}.}.
    \[
    \begin{array}{rll}
...
\end{definition}
    ...
    %De forma equivalente, la gramática previamente mostrada tiene una representación en un conjunto de juicios lógicos.
     \begin{definition}[Definición de la sintaxis concreta para expresiones ariméticas de EAB] \label{sec:sintaxis_concreta_EAB} En su representación como juicios lógicos\footnote{ Definición extraída de \hyperlink{12}{[12]}.}: 
        \[
            \begin{array}{cccc}
            \inference{}{0\;{\text{D}}}&
            \inference{}{1\;{\text{D}}}&
...
A las expresiones de \textsf{EAB} formuladas a partir de la sintaxis concreta, las representaremos con el juicio $ e\ E $ que se lee como: ''e es una expresión de \textsf{EAB}.''
   ...
   \subsection{Análisis sintáctico}
    ...
    En general el analizador sintáctico de un lenguaje de programación debe de proveer con un $ASA$ a todas las expresiones del mismo y este debe de ser libre de ambigüedad. Formalmente, para \textsf{EAB}, esto se representa como:
\begin{itemize}
...
Para este propósito, marcaremos con paréntesis la expresión inmediata a evaluar después del marcador \texttt{then}.
    ...
    \label{sec:sintaxis_concreta_IF}
    \begin{exercise}        
        Define la sintaxis concreta para extender nuestro lenguaje \textsf{EAB} con las instrucciones lógicas previamente discutidas\footnote{Definición formulada a partir de \hyperlink{107}{[107]}}.
        \[ 
	  ...
	  %\inference{c \text{ E } t \text{ E}}{\texttt{if $c$ then ($t$) } \text{E}}  \quad
	  \inference{x \text{ B }}{x \text{ E}}
        \] \\
        %\[
         %   \inference{e \text{ E}}{(e) \text{ LE}} \inference{e \text{ LE}}{ e \text{ E}}
        %\]
	% Antes, la F era LE. lo cambie porque las F pueden usar el perentizado y comenté la regla de arriba para no repetir el mismo mecanismo pero creo que funciona.
	Nótese que el operador \textsf{if} forza el encapsulamiento de la expresión \textsf{then} empleando parentizado explícito. Esta regla podría hacer uso de una estrategia ó heurística para eliminar la ambigüedad del operador y los paréntesis.\\\\
	Por simplicidad y para facilitar la interpretación de la jerarquía de condicionales, utilizaremos el parentizado explicito.
    \end{exercise}

Notemos que con la sintaxis extendida ahora podemos tener expresiones que son sintácticamente correctas, pero que no tienen sentido como: \[ \texttt{true} + 1\]
...
%        \]
    ...
    \begin{exercise}
        ...
        \[
            \inference{\texttt{true} \text{ B} }{\textit{bool[true]} \textit{ ASA}} \quad
            \inference{\texttt{false} \text{ B} }{\textit{bool[false]} \textit{ ASA}}
        \]
        \[
    ...
    \begin{exercise}
        ...
        \[
            \inference{}{\texttt{true} \text{ B} \longleftrightarrow \textit{bool[true]} \textit{ ASA}}[\textit{(true)}]  \quad
            \inference{}{\texttt{false} \text{ B} \longleftrightarrow \textit{bool[false]} \textit{ ASA}}[\textit{(false)}]
        \]
        \[
...
\section{El operador let}
    ...
    La intuición de esta clase de expresiones es que en su proceso de evaluación las apariciones de la variable \textit{x} en $e_2$, serán sustituidas por el resultado de la evaluación de $e_1$.\\\\
...
Definamos entonces la sintaxis concreta de la nueva instrucción como sigue.
    \begin{exercise}
        Proporciona las sintaxis concreta\footnote{Recordemos que las categorías gramaticales B (\hyperref[sec:sintaxis_concreta_IF]{definición 3.1}) y F (\hyperref[sec:sintaxis_concreta_EAB]{definición 1.2}) engloban la definición de los booleanos y los factores en la gramática concreta respectivamente.} para agregar la instrucción \texttt{let} de tal forma que sea congruente con las especificación mencionada anteriormente\footnote{Definición formulada a partir de \hyperlink{1}{[1]}, \hyperlink{5}{[5]}, \hyperlink{12}{[12]} y \hyperlink{81}{[81]} }.
...
La sintaxis abstracta de orden superior, abstrae el ligado como parte del metalenguaje con el que se define el \textit{ASA}, es decir el árbol mismo delimita el alcance de la variable ligada a este.\\\\
    ...
    \begin{exercise}
         ...
         \[
           %\inference{}{x \text{ identificador} \longleftrightarrow \text{$x$ \textit{ASA}}}[\textit{ vara }] \quad
	%\text{$x$ V} \quad             
    ...
    \begin{exercise}
        ...
        \[
            \textit{let(if(bool[true], num[1], num[0]), $y$.if($x$, bool[false], bool[true]))\ ASA}
        \]
    \end{exercise}
    ...
    \begin{exercise}
        ...
        \[
            \textit{let(bool[false], x.if(x, if($\alpha$, if($\alpha$, num[4], num[1]), num[0]), bool[false])) ASA}
        \]        
    \end{exercise}
    ...
    \begin{definition}[Variable de ligado]  La instancia de ligado de una variable es aquella que da a esta su valor. Es decir, es la aparición de la variable en donde esta es definida\footnote{Definición formulada de \hyperlink{2}{[2]}, \hyperlink{5}{[5]}, \hyperlink{12}{[12]}, \hyperlink{80}{[80]} y \hyperlink{81}{[81]}}. \\\\
    ...
    \begin{definition}[Variable ligada] Una variable está ligada si se encuentra contenida dentro del alcance de una variable de ligado con su nombre\footnote{Definición formulada de \hyperlink{2}{[2]}, \hyperlink{5}{[5]}, \hyperlink{12}{[12]}, \hyperlink{80}{[80]} y \hyperlink{81}{[81]}}.\\\\
...
\begin{definition}[Alcance de variable] El alcance de una variable es la región de un programa en la que esta obtiene su valor\footnote{Definición formulada de \hyperlink{5}{[5]} y  \hyperlink{12}{[12]}}. \\\\
...
\end{definition}
    ...
    De tal forma que si tenemos la siguiente expresión: \[ \texttt{let } x = e_1 \texttt{ in } \setulcolor{red}\textit{\ul{y}} + 1 \texttt{ end } \] Decimos que $y$ es una variable libre en nuestra expresión \texttt{let}.
    \end{definition}
    ...
    %		\texttt{let }\setulcolor{purple}\texttt{\ul{x}} = (2 + 4) \texttt{ in } x+1 \texttt{ end }
       % \end{align*}
		...
		\begin{center}
		 \scalemath{0.6}{
		\begin{turn}{0}
		\begin{tabular}{ |p{7.0cm}|p{2.3cm}|p{5.2cm}|p{6.0cm}|  }
		 		\hline
				...
				\hline\xrowht[()]{30pt}
					...
					} &  $\setulcolor{purple}\texttt{\ul{x}}$  &  $2 + 4$   &  $\setulcolor{purple}\texttt{\ul{x}}+1$\\
				\hline\xrowht[()]{40pt}
				\makecell[l]{ 
					\\
					\texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end} }  &   \setulcolor{orange}\texttt{\ul{z}} & 7 &  y + \setulcolor{orange}\texttt{\ul{z}}\\
				\hline\xrowht[()]{60pt}
		 		\makecell[l]{ 
				 \\ 
				 $\texttt{let } y = (x \ast 2) + x \texttt{ in }$ \\
				 \qquad $(x \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z$ \\
				 \texttt{ end } 
				 } &   $\setulcolor{green}\texttt{\ul{y}} $  &  $ (x \ast 2) + x$  &  $(x \ast  \setulcolor{green}\texttt{\ul{y}}) + \texttt{let }z = 7 \texttt{ in }   \setulcolor{green}\texttt{\ul{y}} + z$\\
				\hline\xrowht[()]{90pt}
				\makecell[l]{
					\\
					$\texttt{let  } x = \texttt{let }x = (2 + 4) \texttt{ in } x+1 \texttt{ end} \texttt{ in }$ \\  
					\qquad $\texttt{let } y = (x \ast 2) + x \texttt{ in }$ \\
					\qquad \qquad $(x \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end}$\\
					\qquad \texttt{ end } \\ 
					\texttt{ end } \\
				} & $\setulcolor{blue}\texttt{\ul{x}}$ &  \makecell[l]{ \texttt{let }x = (2 + 4) \texttt{ in } x+1 \texttt{ end} }  & \makecell[l]{ $ \texttt{let } y = (\setulcolor{blue}\texttt{\ul{x}} \ast 2) + \setulcolor{blue}\texttt{\ul{x}} \texttt{ in }$ \\
					\qquad $(\setulcolor{blue}\texttt{\ul{x}} \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end}$\\
					\texttt{ end } }\\
					\hline
		\end{tabular}
		\end{turn}
...
%	\scalemath{0.60}{
%		\begin{turn}{90}
%			\begin{tabular}{ |p{7.2cm}|p{5.7cm}|p{6.5cm}|p{6.5cm}|  }
%				\hline
%				\multicolumn{4}{|c|}{Expresiones \texttt{let} en \textsf{EAB}} \\
%				\hline\xrowht[()]{20pt}
%				Expresión & Variable de ligado & Alcance & Variable ligada\\
%				\hline\xrowht[()]{30pt}
%				\makecell[l]{ \\ $\texttt{let } x = (2 + 4) \texttt{ in } x+1 \texttt{ end}$ \\
%				} &  $\setulcolor{purple}\texttt{\ul{x}} = (2 + 4)$  &  $x+1$   &  $\setulcolor{purple}\texttt{\ul{x}}+1$\\
%				\hline\xrowht[()]{40pt}
%				\makecell[l]{ 
%					\\
%					\texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end} }  &   \setulcolor{orange}\texttt{\ul{z}} = 7  & y + z &  y + \setulcolor{orange}\texttt{\ul{z}}\\
%				\hline\xrowht[()]{60pt}
%				\makecell[l]{ 
%					\\ 
%					$\texttt{let } y = (x \ast 2) + x \texttt{ in }$ \\
%					\qquad $(x \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z$ \\
%					\texttt{ end } 
%				} &   $\setulcolor{green}\texttt{\ul{y}} = (x \ast 2) + x$  &  $(x \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z$  &   $(x \ast  \setulcolor{green}\texttt{\ul{y}}) + \texttt{let }z = 7 \texttt{ in }   \setulcolor{green}\texttt{\ul{y}} + z$\\
%				\hline\xrowht[()]{90pt}
%				\makecell[l]{
%					\\
%					$\texttt{let  } x = \texttt{let }x = (2 + 4) \texttt{ in } x+1 \texttt{ end} \texttt{ in }$ \\  
...
%					\qquad \texttt{ end } \\ 
%					\texttt{ end } \\
%				} & $\setulcolor{blue}\texttt{\ul{x}} = \texttt{let }x = (2 + 4) \texttt{ in } x+1 \texttt{ end}$ &  \makecell[l]{ $\texttt{let } y = (x \ast 2) + x \texttt{ in }$ \\
%					\qquad $(x \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end}$ \\
%					\texttt{ end } }  & \makecell[l]{ $ \texttt{let } y = (\setulcolor{blue}\texttt{\ul{x}} \ast 2) + \setulcolor{blue}\texttt{\ul{x}} \texttt{ in }$ \\
%					\qquad $(\setulcolor{blue}\texttt{\ul{x}} \ast y) + \texttt{let }z = 7 \texttt{ in }  y + z \texttt{ end}$\\
%					\texttt{ end } }\\
%				\hline
%			\end{tabular}
...
%\end{center}
    ...
    \begin{exercise}
        Al evaluar la expresión $e$ se puede obtener un valor? si sí muestra el procedimiento para la evaluación informal (aún no definimos la semántica operacional para \texttt{let} en \textsf{EAB}, pero se puede esbozar el método de evaluación partiendo de la descripción de  instrucción.)
...
\section{Sustitución y $\alpha$-equivalencias}
    ...
    \begin{definition}[Función de sustitución para \textsf{EAB}] Se define la función de sustitución\footnote{Esta definición está formulada a partir de \hyperlink{1}{[1]}, \hyperlink{2}{[2]}, \hyperlink{5}{[5]} y \hyperlink{12}{[12]} adaptada para las instrucciones que hemos revisado hasta este momento para \textsf{EAB}.} sobre los árboles de sintaxis abstracta de \textsf{EAB}:
        \[
            \begin{array}{lclr}
                x[x :=e]&=&e&\\
                z[x :=e]&=&z&\\
	     \textit{bool[true]}\; [z:=e]& =& \textit{bool[true]} \\
	     \textit{bool[false]}\; [z:=e] &=& \textit{bool[false]}\\
                num[n][x:=e]&=&num[n]&\\
    ...
    \begin{exercise}
        Desarrolla la sustitución aplicada a la siguiente expresión de acuerdo con la función de sustitución previamente definida.
        	...
        	\]
       	%\[ 
	%	\textit{let(prod(sum(w, num}[1]), \textit{num}[2]\textit{)}, x.\textit{if(} \textit{T, } w, \textit{ prod(2, x)))} \; [ w := \textit{num}[2]] 
	%\]
	%\[
	%	\textit{let(prod(sum(w, num}[1]), \textit{num}[2]\textit{)}[ w := \textit{num}[2]] , x.\textit{if(} \textit{T, } w, \textit{ prod(2, x))}[ w := \textit{num}[2]])  
	%\]
	%\[
	%	\textit{let(prod(sum(w, num}[1])[ w := \textit{num}[2]], \textit{num}[2][ w := \textit{num}[2]] \textit{)}, x.\textit{if(} \textit{bool[true]}[ w := \textit{num}[2]], w \ [ w := \textit{num}[2]], \textit{ prod(}num[2], x)[ w := \textit{num}[2]] ))  
	%\]
	%\[
	%	\textit{let(prod(sum(}w[ w := \textit{num}[2]], num[1][ w := \textit{num}[2]]), \textit{num}[2] \textit{)}, x.\textit{if(} \textit{bool[true]}, \ num[2], \textit{ prod(num}[2][ w := \textit{num}[2]], \  x[ w := \textit{num}[2]]))) 
	%\]
	%\[
	%	\textit{let(prod(sum(}\textit{num}[2], num[1]), \textit{num}[2] \textit{)}, x.\textit{if(} \textit{bool[true]}, \ num[2], \textit{ prod(num}[2], \  x))) 
	%\]
	%\[ 
	%	\scalemath{0.9}{
	%		\textit{let(prod(sum(num}[2], \textit{num}[1]), \textit{num}[2]), x.\textit{if(} \textit{T, }  \textit{num}[2], \textit{ prod(num}[2], x))
	%	}
	%\]
...
\flushleft{
	\rotatebox{90}{
		\parbox{20cm}{
  			$\textit{let(prod(sum(w, num}[1]), \textit{num}[2]\textit{)}, x.\textit{if(} \textit{bool[true]}, w, \textit{ prod(2, x)))} \; [ w := \textit{num}[2]]$
		}
	}
}
\end{minipage}
...
\flushleft{
	\rotatebox{90}{
		\parbox{20cm}{
  			$\textit{let(prod(sum(w, num}[1]), \textit{num}[2]\textit{)}[ w := \textit{num}[2]] , x.\textit{if(} \textit{bool[true], } w, \textit{ prod(2, x))}[ w := \textit{num}[2]])$
		}
	}
}
\end{minipage}
