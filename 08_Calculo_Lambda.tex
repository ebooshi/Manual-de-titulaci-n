...
\section{Sintaxis del Cálculo Lambda}
    ...
    \begin{itemize}
        \item \textbf{Variables:} los elementos de esta categoría pertenecen a un conjunto infinito de letras minúsculas, generalmente las últimas del alfabeto (y en ocasiones pueden estar numeradas, algunos ejemplos pueden ser: $x$, $y$, $z$, $x_1$, $z_2$, etc.) y son las lambda-expresiones más simples.\\
        ...
        \item \textbf{Aplicación:} Esta operación se representa como: ''$e_1\,e_2$'' y corresponde a la aplicación de la función e1 a la función e2. La aplicación es asociativa a la izquierda de modo que una expresión como $e_1\ e_2\ e_3 $ se interpreta como $(e_1 e_2) e_3$.
    \end{itemize}

    \begin{definition}[Sintaxis concreta del Cálculo Lambda] Definimos el juicio $e \; \lambda term$ que indica que $e$ es una expresión válida de acuerdo a la sintaxis del Cálculo Lambda. A este tipo de términos los denominaremos  $\lambda-$expresiones\footnote{Definición extraída de \hyperlink{5}{[5]},  \hyperlink{12}{[12]}, \hyperlink{108}{[108]}, \hyperlink{109}{[109]}}.
...
\bigskip
    ...
    \begin{exercise}
       ...
       \begin{center}
          \begin{tabular}{ c c }
             ...
             $x$ 			  	 	  & \text{variable} \\ 
             $y$		 	 	 	  & \text{variable}  \\
             $\lambda x.x$	  		  & \text{lambda abstracción} \\
             $\lambda x.y$   	  		  & \text{lambda abstracción}  \\
             ($\lambda x.x)v$  	    	  & \text{aplicación} \\
             ($\lambda x.x)v'$	 	 	  & \text{aplicación} \\
             $\lambda x.\lambda y.xy$           & \text{lambda abstracción}\\
    ...
    En el ejemplo anterior, el tercer caso corresponde a la función identidad que regresa el mismo argumento que recibe. El cuarto corresponde a la función constante que siempre regresa el mismo valor y el séptimo es un ejemplo de como se puede anidar lambda abstracciones mono-parametrizadas para incrementar el número de parámetros en el cuerpo de la expresión\footnote{A este proceso se le conoce como ''\textit{currificación}'' en honor a Haskell Curry y nos permite anidar tantas funciones como parámetros necesitemos.}. 
    
	\subsubsection{Currificación en el Cálculo Lambda}
	
	Dado que la definición sintáctica del Cálculo Lambda sólo permite abstracciones
	con un único parámetro, las funciones de múltiples argumentos no son primitivas,
	sino que se representan mediante abstracciones anidadas.
	
	La \emph{currificación} es el principio según el cual una función de $n$ parámetros
	puede expresarse como una sucesión de $n$ funciones unarias. 
	Formalmente, si $M$ es un término y $x_1, \dots, x_n$ son variables, entonces
	una función de $n$ parámetros se escribe como:
	
	\[
	\lambda x_1.\lambda x_2.\dots\lambda x_n.\, M.
	\]
	
	Cada abstracción introduce una variable ligada, y la aplicación de varios argumentos
	se entiende como aplicación iterada asociativa por la izquierda. De esta manera, un término como:

	\[
	\lambda x.\lambda y.\, xy
	\]
	
	representa una función de dos parámetros construida exclusivamente a partir
	de abstracciones unarias anidadas. En general, este mecanismo permite representar
	funciones de cualquier aridad sin extender la sintaxis básica del lenguaje.


\section{$\alpha$-equivalencia en el Cálculo Lambda}

    En el Cálculo Lambda se tiene un constructor similar al operador \textsf{let} que tiene una variable ligada y un alcance asociado a ella. Esto nos permite definir la $\alpha$-equivalencia para expresiones que difieren a lo más en el nombrado de sus variables ligadas. 
...
A la operación de sustituir los términos que concuerden con la variable del operador en la $\lambda-$expresión se le conoce como $\beta$-reducción y se representa con el símbolo $\to_\beta$. 
 ...
 En el Cálculo Lambda las funciones son los únicos objetos primitivos del sistema, no existen constantes, estructuras de datos ni tipos básicos incorporados en la sintaxis. Esto significa que las funciones:
 \begin{enumerate}
 	\item Constituyen las entidades fundamentales a partir de las cuales se construyen todos los términos del lenguaje.
 	\item Pueden recibir otras funciones como argumentos y devolver funciones como resultado.
 \end{enumerate}
 
 Esta propiedad caracteriza al Cálculo Lambda como un sistema de orden superior, 
 pues las funciones no sólo actúan sobre valores, sino también sobre otras funciones.\\\\
 La definibilidad lambda entonces es consecuencia directa del sistema, por ejemplo, 
 cualquier objeto que deseemos introducir en el sistema, como: números, valores booleanos 
 o estructuras más complejas. En el Cálculo Lambda,
 estos tipos primitivos serán definidos mediante funciones anónimas a las que 
 convendremos un nombre para facilitar su representación\\
 
 En contraste, en los lenguajes de programación convencionales las entidades o tipos 
 primitivos son proporcionados directamente por la biblioteca estándar del lenguaje 
 (\textsf{int}, \textsf{char}, \textsf{bool}, etc.).\footnote{Información consultada de \hyperlink{108}{[108]} y  \hyperlink{109}{[109]}}.
 ...
 En el Cálculo Lambda se espera que los operadores anteriormente definidos 
 satisfagan las siguientes propiedades de comportamiento, en el sentido de que, 
 tras aplicar un número finito de $\beta$-reducciones ($\to_\beta$), los términos 
 correspondientes se reduzcan a las expresiones indicadas a la derecha:
...
%https://stackoverflow.com/questions/29756732/how-would-the-lambda-calculus-add-numbers
    \subsubsection{Suma}
        ...
        \begin{definition}[Definición de la suma para los numerales de Church]\footnote{Definición extraída de \hyperlink{140}{[140]}, \hyperlink{109}{[109]} y  \hyperlink{111}{[111]}}
...
\begin{array}{cl}
	...
	\textsf{absDiff } m\ n =_{def} & \textsf{add} (\textsf{sub}\ n\ m)\ (\textsf{sub}\ m\ n) \\
	\textsf{equal } m\ n =_{def} & \textsf{isZero} (\textsf{absDiff}\ m \ n) \\
...
\end{definition}
  ...
  \section{Datos estructurados en el Cálculo Lambda}
	 ...
	 La estructura más simple que revisaremos serán las tuplas binarias (pares) junto con sus proyecciones \textsf{first} y \textsf{second} que ayudarán a sentar la base para definir a las listas con sus respectivas funciones para obtener la cabeza y la cola. 
...
\[
    ...
    \]
            ...
            \begin{definition}[Definición de listas en Cálculo Lambda]  Las listas en el Cálculo Lambda se apoyan del constructor de los pares para ir añadiendo elementos anidando esta operación agregando una unidad por aplicación. \\\\
...
entonces $e_f = e_f'$ salvo $\alpha$-equivalencias. 
        ...
        \section{Combinadores de punto fijo}
                   ...
                   Para este fin se precisa de la introducción de los combinadores de punto fijo que capturan la esencia del principio de recursión general en computación:
                    ...
                    \begin{definition}[Combinador de punto fijo] Un $\lambda$-expresión cerrada $C$ es un combinador de punto fijo sí y sólo si cumple alguna de las siguientes condiciones\footnote{Definición formulada de \hyperlink{5}{[5]}, \hyperlink{12}{[12]} y \hyperlink{113}{[113]}}:
                        \begin{enumerate}
...
\]
            ...
            \section{Ejercicios para el lector}
                ...
                \begin{exercise}
                    Supón que se requiere extender la definición 5.6) para booleanos y sus operadores en el Cálculo Lambda implementando el siguiente operador:
                    \[\textsf{OR}\ a\ b\]
