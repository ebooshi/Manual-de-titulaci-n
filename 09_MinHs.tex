...
Con la teoría que hemos revisado hasta el momento en nuestro lenguaje \textsf{EAB} cuya definición engloba a los números naturales, booleanos, instrucciones lógicas y el operador let a la par del Cálculo Lambda, nos es natural preguntarnos  ¿Cómo es la implementación de un lenguaje de programación de características similares para una computadora moderna?
Para poder contestar a esta pregunta tomaremos como caso de estudio una versión simplificada del lenguaje de programación \textsf{Haskell}. Este posee características importantes para el enfoque de este curso que serán de interés trasladar a \textsf{EAB}. \\\\
...
\subsection{Sintaxis concreta}
    El lenguaje de programación \textsf{Haskell} emplea la declaración explícita de tipos en su sintaxis concreta, este patrón se traslada a \textsf{MinHS} mediante la introducción de las anotaciones de tipos para \textit{Int}, \textit{Bool} y en el caso de las funciones, su tipo será representado por la expresión $T_1 \rightarrow T_2$ en la sintaxis concreta donde $T_1$ representa el tipo de la variable ligada en el cuerpo de la expresión  y $T_2$ es el tipo que regresa la evaluación de la expresión.\\\\
Esta modificación a la sintaxis concreta solo aplicará para aquellas expresiones donde sea estrictamente necesario anotar su tipo para verificar la correctitud de tipificado. El resto de las expresiones se definen de manera similar a como lo hicimos para \textsf{EAB} limitando la aparición de típo en la medida de lo posible.
...
\section{Semántica de \textsf{MinHs}}
    ...
    En los lenguajes de programación los sistemas de tipos brindan información adicional sobre la evaluación de las expresiones. Por ejemplo, que valores debemos esperar recibir y regresar al concluir la ejecución de nuestro programa para definir restricciones que nos permitan tener seguridad y congruencia en los datos\footnote{definición formulada a partir de \hyperlink{96}{[96]}}.\\\\
        ...
        \begin{description}
            ...
            \[
                \begin{array}{ccc}
                    ...
                    &&\\
                    \inference{\Gamma\vdash t_1:Nat&\Gamma\vdash t_2: \textit{Nat}}{\Gamma\vdash res(t_1,t_2) : \textit{Nat}}&
                    \quad&
...
En el contexto de \textsf{MinHS} nos interesa que a  una expresión correctamente construida y correctamente tipificada se le pueda aplicar alguna de las reglas definidas por la semántica dinámica (progreso). Adicionalmente nos interesa que el tipo de las expresiones sea único y que a cada paso en la evaluación este se conserve (preservación). A continuación daremos una definición formal para estas propiedades.
        ...
        \begin{itemize}
            \item \textbf{Progreso:} un programa que cumple con las restricciones del sistema de tipos no se bloquea. % Este comportamiento está capturado en la definición de progreso enlistada a continuación para las expresiones de \textsf{MinHS}:
		   ...
		   \end{itemize}
            ...
            \item \textbf{Preservación:} un programa que cumple con las restricciones del sistema de tipos dará como resultado de la evaluación una expresión correctamente tipificada. Más aún, el tipo resultante será el mismo que aquél con el que se inició la evaluación. %Esto se puede observar en las siguientes dos propiedades definidas para las expresiones de \textsf{MinHS}:
...
\bigskip
    ...
    \begin{exercise}
        ...
        \begin{itemize}
            ...
            \item \textsf{fun} x ::\ $\textit{Nat}\ \Rightarrow\ x +1$\;  
            \item  $ \textsf{recfun}\; \textit{fact}\; ::\; (\textit{Nat} \rightarrow \textit{Nat}\textsf)\; x \Rightarrow\; \textsf{if}\; \textsf{IsZero($x$)}\; \textsf{then}\; 1\; \textsf{else}\; n\; *\; \textit{fact}\; (n-1)$
