...
\begin{exercise}
	Proporciona la definición de una clase \textsf{MinHeap} que modele una
	estructura de datos de tipo heap mínimo. Puedes suponer la existencia
	de la clase \textsf{Integer} que permite comparar valores.
	
	\begin{verbatim}
		class HeapNode extends Object {
			Integer value;
			HeapNode left;
			HeapNode right;
			
			HeapNode(Integer value, HeapNode left, HeapNode right){
				super();
				this.value = value;
				this.left = left;
				this.right = right;
		...
		class MinHeap extends Object {
			HeapNode root;
			
			MinHeap(HeapNode root){
				super();
				this.root = root;
			}
...
\begin{definition}[Semántica Estática de \textsf{Java Peso Pluma}] A continuación enunciamos las reglas de tipificado\footnote{Definición formulada de \hyperlink{5}{[5]}, \hyperlink{12}{[12]}, \hyperlink{128}{[128]}, \hyperlink{129}{[129]} y \hyperlink{132}{[132]} }\\
...
\begin{description}
	...
	Es decir, si la clase {\tt C} extiende a la clase {\tt D} entonces {\tt C} es subtipo de {\tt D}.\\
        La relación de subtipificado debe cumplir las propiedades de transitividad y reflexividad discutidas en el capítulo anterior.
...
\begin{proposition}[Preservación de tipos]\label{preservación} Si $T$ es una tabla de clases bien formada, $\Gamma\vdash e:C$ y $e\rightarrow_{\tt fj} e'$, entonces existe $C'$ tal que $C'\leq C$ y $\Gamma\vdash e' : C'$\footnote{Definición formulada de \hyperlink{12}{[12]} y \hyperlink{128}{[128]} }
\end{proposition}
...
\begin{proposition}[Progreso de la relación $\rightarrow_{\tt fj}$]\label{progreso} Sea $T$ una tabla de clases bien formada, si $\varnothing\vdash e:C$ entonces sucede una y solo una de las siguientes condiciones:
\begin{itemize}
	\item $e$ es un valor.
...
\begin{proposition}[Seguridad de \textsf{Java Peso Pluma}] Si $T$ es una tabla de clases bien formada, $\varnothing\vdash e:C$ y $e \rightarrow_{\tt fj}^\star e'$ con $e'$ en forma normal, entonces se cumple una y solo una de las siguientes condiciones:
\begin{itemize}
...
A lo largo de este manual se presentaron los conceptos fundamentales que permiten estudiar a los lenguajes de programación desde una perspectiva formal. Más allá de entenderlos únicamente como herramientas prácticas para escribir programas, el objetivo fue analizarlos como estructuras bien definidas que pueden describirse mediante modelos matemáticos y reglas precisas.\\\\
El recorrido comenzó introduciendo algunas herramientas matemáticas básicas que nos permiten razonar sobre los programas y sus propiedades. A partir de estas ideas fue posible presentar la sintaxis y la semántica de un lenguaje, dos componentes esenciales que determinan tanto la forma en la que se escriben los programas como el significado que tienen cuando se ejecutan.\\\\
Posteriormente se estudiaron distintos modelos que ayudan a comprender el comportamiento de los programas. El cálculo lambda permitió analizar la noción de función y evaluación desde un punto de vista abstracto, mientras que el lenguaje \textsf{MinHS} sirvió como un primer ejemplo concreto de cómo estas ideas pueden materializarse en un lenguaje funcional. De manera complementaria se discutió el problema de la inferencia de tipos, mostrando cómo es posible deducir propiedades importantes de los programas a partir de reglas formales.\\\\
Más adelante se introdujeron modelos operacionales para describir la ejecución de programas mediante máquinas abstractas, así como el lenguaje imperativo \textsf{TinyC}, que incorpora el manejo explícito de estado y memoria. Finalmente se exploraron conceptos propios del paradigma orientado a objetos, como la herencia y el subtipado, culminando con el estudio de \textit{Featherweight Java}, un modelo que captura los elementos esenciales de este estilo de programación.\\\\
En conjunto, los temas presentados buscan ofrecer una visión más clara de los principios que dan forma a los lenguajes de programación modernos. Comprender estos fundamentos no solo permite analizar con mayor profundidad los lenguajes existentes, sino también contar con las herramientas necesarias para estudiar, diseñar e incluso proponer nuevos lenguajes de programación.